 ...
 *
 * @note All references to the standard in this file cite ETSI TS 103 636 DECTNRP NR+ V2.2.1
 * (2026-05).
 *
...
struct dectnrp_rx_mode {
	...
	uint8_t is_scheduled: 1;
	/** The rx operation wants to receive a beacon. The driver may not be interested in this
	 * field. */
	uint8_t is_beacon: 1;
};
...
struct dectnrp_driver_op {
	...
	union {
		...
		struct {
			...
			uint32_t subslots;
			/** Reference to result storage where measured RSSI1 values should be
			 * stored. Memory has to be kept available by the caller until
			 * DECTNRP_EVENT_OP_RESULTS is notified.
			 */
...
struct dectnrp_driver_event {
	...
	union {
		...
		struct {
			 ...
			 *
			 * @todo It needs to be decided whether we want to transmit the RSSI2
			 * according to spec.
			 */
			uint8_t rssi2;
			 ...
			 *
			 * @todo It needs to be decided whether we want to transmit the SNR
			 * according to sp
			 */
			int8_t snr;
		} message_error;
...
typedef void (*dectnrp_driver_event_cb_t)(const struct device *dev,
...
struct dectnrp_config {
	...
	union {
		...
		struct {
			 ...
			 * @attention This only applies to packages of PHY TYPE 2! Packages are
			 * filtered by the receiver identity encoded in the PHY TYPE 2 header. PHY
			 * TYPE 1 packages have not short address attached and are not be filtered!
			 */
			uint16_t value;
