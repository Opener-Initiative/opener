...
LOG_MODULE_REGISTER(dectnrp_nrf91x1, CONFIG_DECTNRP_DRIVER_NRF91X1_LOG_LEVEL);
...
/**
 * @brief The context related to the one and only open nrf91x1 radio device.
 */
struct nrf91x1_context {

	/** Interface related to this nrf91x1 radio device. */
	struct net_if *iface;
	...
	/** State of this nrf91x1 radio device. */
	atomic_t state;

	/** Marks the modem firmware version. */
	enum nrf91x1_mfw_version mfw_version;
...
static const struct device *nrf91x1_dev = NULL;
 ...
 * @return uint8_t
 */
static inline uint8_t dectnrp_nrf91x1_encode_rssi2(int16_t rssi2)
{
	rssi2 = rssi2 >> 1;
 ...
 * @return int8_t
 */
static inline int8_t dectnrp_nrf91x1_encode_snr(int16_t snr)
{
 ...
 *
 * @param ctx The context related to nrf91x1 radio device.
 * @param time Modem time of the event.
 * @param event 'NRF_MODEM_DECT_PHY_EVT_INIT' event.
 */
static void modem_init_cb(struct nrf91x1_context *ctx, const uint64_t *time,
...
/**
 * @brief Event callback returning the state after modem deinitialization by
 ...
 */
static void modem_deinit_cb(struct nrf91x1_context *ctx, const uint64_t *time,
...
/**
 * @brief Event callback returning the state after modem configuration by
 * nrf_modem_dect_phy_configure.
 *
 * @attention This function should be used in blocking mode, it releases ctx->modemlock!
 *
 * @param ctx The context related to nrf91x1 radio device.
 ...
 */
static void modem_configure_cb(struct nrf91x1_context *ctx, const uint64_t *time,
			       const struct nrf_modem_dect_phy_configure_event *event)
{
	if (event->err == NRF_MODEM_DECT_PHY_SUCCESS) {
		LOG_DBG("<%lld> PHY_EVT_CONFIGURE success,%d°C,%umV", *time, event->temp,
			event->voltage);
	} else {
		LOG_ERR("PHY_EVT_CONFIGURE failed,0x%x,%d°C,%umV", event->err, event->temp,
			event->voltage);
	}
	ctx->modem.time = *time;
	ctx->modem.temperature = event->temp;
...
/**
 * @brief Event callback returning the state after radio configuration by
 * nrf_modem_dect_phy_radio_config.
 *
 ...
 */
static void modem_radio_config_cb(struct nrf91x1_context *ctx, const uint64_t *time,
 ...
 */
static void modem_activate_cb(struct nrf91x1_context *ctx, const uint64_t *time,
 ...
 */
static void modem_deactivate_cb(struct nrf91x1_context *ctx, const uint64_t *time,
...
/**
 * @brief Event callback notifying modems capabilities requested by
 ...
 */
static void modem_capability_get_cb(struct nrf91x1_context *ctx, const uint64_t *time,
				    const struct nrf_modem_dect_phy_capability_get_event *event)
{
	if (event->err == NRF_MODEM_DECT_PHY_SUCCESS) {
		...
		if (IS_ENABLED(CONFIG_DECTNRP_DRIVER_NRF91X1_CAP_LOGGING)) {
...
/**
 * @brief Event callback notifying supported bands and there parameters requested by
 * nrf_modem_dect_phy_band_get.
 *
 * @attention This function should be used in blocking mode, it releases ctx->modemlock!
 *
 ...
 */
static void modem_band_get_cb(struct nrf91x1_context *ctx, const uint64_t *time,
			      const struct nrf_modem_dect_phy_band_get_event *event)
{
	if (event->err == NRF_MODEM_DECT_PHY_SUCCESS) {
		if (IS_ENABLED(CONFIG_DECTNRP_DRIVER_NRF91X1_CAP_LOGGING)) {
			LOG_WRN("<%lld> %u bands supported:0x%x.", *time, event->band_count,
				event->supported_bands);
 ...
 */
static void modem_latency_get_cb(struct nrf91x1_context *ctx, const uint64_t *time,
 ...
 */
static void modem_time_get_cb(struct nrf91x1_context *ctx, const uint64_t *time,
 ...
 */
static void modem_rssi_result(struct nrf91x1_context *ctx, const uint64_t *time,
 ...
 */
static void modem_pcc_received(struct nrf91x1_context *ctx, const uint64_t *time,
 ...
 */
static void modem_pcc_crc_err(struct nrf91x1_context *ctx, const uint64_t *time,
			      const struct nrf_modem_dect_phy_pcc_crc_failure_event *event)
{
	...
	if (ctx->upper_layer_event_handler) {
		...
		struct dectnrp_driver_event stack_event = {
			...
			.message_error.rssi2_valid =
				(rssi2 != NRF_MODEM_DECT_PHY_RSSI2_NOT_MEASURED),
			.message_error.snr_valid =
				(event->snr != NRF_MODEM_DECT_PHY_SNR_NOT_MEASURED)};

		if (stack_event.message_error.rssi2_valid) {
			/* Transcode rssi2 value */
			stack_event.message_error.rssi2 = dectnrp_nrf91x1_encode_rssi2(rssi2);
		}
 ...
 */
static void modem_pdc_received(struct nrf91x1_context *ctx, const uint64_t *time,
	...
	if (!atomic_test_bit(&ctx->rx.state, RADIO_RXSTATE_PCC)) {
		...
		if (ctx->upper_layer_event_handler) {
			struct dectnrp_driver_event stack_event = {
				...
				.msg_received.rssi2_valid =
					(rssi2 != NRF_MODEM_DECT_PHY_RSSI2_NOT_MEASURED),
				.msg_received.snr_valid =
					(event->snr != NRF_MODEM_DECT_PHY_SNR_NOT_MEASURED)};

			if (stack_event.msg_received.rssi2_valid) {
				/* Transcode rssi2 value */
				stack_event.msg_received.rssi2 =
					dectnrp_nrf91x1_encode_rssi2(rssi2);
			}
			if (stack_event.msg_received.snr_valid) {
				/* Transcode snr value */
				stack_event.msg_received.snr =
 ...
 */
static void modem_pdc_crc_err(struct nrf91x1_context *ctx, const uint64_t *time,
			      const struct nrf_modem_dect_phy_pdc_crc_failure_event *event)
{
	...
	if (!atomic_test_bit(&ctx->rx.state, RADIO_RXSTATE_PCC)) {
		...
		if (ctx->upper_layer_event_handler) {
			...
			if (stack_event.message_error.rssi2_valid) {
				/* Transcode rssi2 value */
				stack_event.message_error.rssi2 =
					dectnrp_nrf91x1_encode_rssi2(rssi2);
			}
			if (stack_event.message_error.snr_valid) {
				/* Transcode snr value */
				stack_event.message_error.snr =
					dectnrp_nrf91x1_encode_snr(event->snr);
			}
 ...
 */
static void modem_op_complete_cb(struct nrf91x1_context *ctx, const uint64_t *time,
...
/**
 * @brief Event callback notifying status of cancel operation requested by
 ...
 */
static void modem_op_cancel_cb(struct nrf91x1_context *ctx, const uint64_t *time,
...
/**
 * @brief Event callback returning the state after configuration of links by
 * nrf_modem_dect_phy_link_config.
 *
 * @attention This function is used in asynchronous mode!
 * @attention This function does only logging up to now!
 *
 ...
 * @param cancel 'NRF_MODEM_DECT_PHY_EVT_CANCELED' event.
 */
static void modem_link_config_cb(struct nrf91x1_context *ctx, const uint64_t *time,
				 const struct nrf_modem_dect_phy_link_config_event *event)
{
	...
	if (event->err == NRF_MODEM_DECT_PHY_SUCCESS) {
		...
		LOG_ERR("<%lld> PHY_EVT_LINK_CONFIG failed,0x%x", *time, event->err);
	}
}
...
{
	struct nrf91x1_context *ctx = nrf91x1_dev->data;
 ...
 * @return int
 */
static int verify_modem_fw_version(enum nrf91x1_mfw_version *mfw_version)
{
	...
	if (ret != 0) {
		...
		if (!fw_version_end) {
			...
			*fw_version_end = '\0';
			...
			char *fw_version_1_1_0 = strstr(fw_version_buf, "mfw-nr+_nrf91x1_1.1.0");
			if (fw_version_1_1_0) {
...
static int nrf91x1_start(const struct device *dev)
{
	int ret = 0;
	struct nrf91x1_context *ctx = dev->data;
	LOG_INF("nrf91x1_start(%s)", dev->name);

	if (!atomic_test_bit(&ctx->state, RADIO_STATE_INITIALIZED)) {
		LOG_ERR("nrf91x1_start but !RADIO_STATE_INITIALIZED");
		ret = -EIO;
		goto error;
		...
		NET_INFO("DECT radio nrf91x1 started");
	}

	goto out;
error:
	NET_INFO("dectnrp_nrf91x1 NOT initialized");
out:
	return ret;
}

static int nrf91x1_stop(const struct device *dev)
{
	struct nrf91x1_context *ctx = dev->data;
	LOG_INF("nrf91x1_stop(%s)", dev->name);
	...
	NET_INFO("DECT radio nrf91x1 stopped");
	goto out;
error:
	NET_INFO("dectnrp_nrf91x1 NOT stopped");
out:
	return ret;
}

static int nrf91x1_get_capabilities(const struct device *dev,
				    struct dectnrp_device_capabilities *caps)
{
	LOG_INF("nrf91x1_get_capabilities(%s)", dev->name);
	caps->hw_caps = (DECTNRP_HW_FILTER | DECTNRP_HW_TXTIME | DECTNRP_HW_RXTIME);
	return 0;
}

static int nrf91x1_configure(const struct device *dev, enum dectnrp_config_type type,
			     const struct dectnrp_config *config)
{
	struct nrf91x1_context *ctx = dev->data;
	LOG_INF("nrf91x1_configure(%s)", dev->name);
...
static int nrf91x1_get_time(const struct device *dev, net_time_t *time)
{
	struct nrf91x1_context *ctx = dev->data;
	LOG_DBG("nrf91x1_get_time(%s)", dev->name);

	int ret = 0;

	if (!atomic_test_bit(&ctx->state, RADIO_STATE_INITIALIZED)) {
		LOG_ERR("nrf91x1_get_time but !RADIO_STATE_INITIALIZED");
		ret = -EIO;
		goto error;
	}
	...
	if (ret < 0) {
		NET_ERR("nrf91x1_get_time() k_sem_take(operation) failed");
		ret = -EIO;
		goto error;
	}
	...
	if (ret < 0) {
		NET_ERR("nrf91x1_get_time() k_sem_take(modem) failed");
		ret = -EIO;
		goto out;
	}

	*time = ctx->modem.time;
	LOG_INF("<%" PRIu64 "> nrf91x1_get_time()", ctx->modem.time);
...
static int nrf91x1_schedule(const struct device *dev, struct dectnrp_driver_op *op)
{
	struct nrf91x1_context *ctx = dev->data;
	LOG_DBG("nrf91x1_schedule(%s)", dev->name);

	int ret = 0;

	if (!atomic_test_bit(&ctx->state, RADIO_STATE_STARTED)) {
		LOG_ERR("nrf91x1-device not initialized yet, drop rx operation");
		ret = -EIO;
		goto out;
	}
	...
	case DECTNRP_DRIVER_OP_TX: {
		...
		if (op->tx.mode.is_lbt) {
			...
			if (op->tx.lbt.period < NRF_MODEM_DECT_LBT_PERIOD_MIN) {
				NET_ERR("[%p]nrf91x1_schedule lbt.period %d to small, min=%d", op,
					op->tx.lbt.period, NRF_MODEM_DECT_LBT_PERIOD_MIN);
				ret = -EINVAL;
				goto out;
			} else if (op->tx.lbt.period > NRF_MODEM_DECT_LBT_PERIOD_MAX) {
				NET_ERR("[%p]nrf91x1_schedule lbt.period %d to big, min=%d", op,
		...
		ret = k_sem_take(&ctx->operation, K_MSEC(10));
		if (ret < 0) {
			NET_ERR("[%p]nrf91x1_schedule k_sem_take failed,%d", op, ret);
			ret = -EIO;
		} else {
...
static int nrf91x1_set_channel(const struct device *dev, uint16_t channel)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(channel);
	return -ENOSYS;
...
static int nrf91x1_rx(const struct device *dev, struct dectnrp_rx_mode mode, net_time_t start,
...
static int nrf91x1_tx(const struct device *dev, struct dectnrp_tx_mode mode, struct net_pkt *pkt,
		      struct net_buf *frag)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(mode);
...
static int nrf91x1_rssi1(const struct device *dev, struct dectnrp_rssi_mode mode, uint32_t subslots,
...
static int nrf91x1_init(const struct device *dev)
{
	int ret = 0;
	struct nrf91x1_context *ctx = dev->data;
	LOG_DBG("nrf91x1_init(%s)", dev->name);

	if (nrf91x1_dev != NULL) {
		/* This driver is only a singleton initialized once for now. */
		LOG_ERR("nrf91x1_init(%s) failed, init twice not supported for now", dev->name);
		ret = -EEXIST;
	} else {
		...
		static const struct nrf_modem_dect_phy_config_params init_params = {
			.band_group_index = CONFIG_DECTNRP_DRIVER_NRF91X1_BAND_GROUP,
		...
		if (ret < 0) {
			...
			NET_ERR("nrf_modem_dect_phy_capability_get failed,0x%x",
				ctx->modem.last_status);
			ret = -EIO;
			goto error;
		}
	...
	LOG_INF("nrf91x1 dectnrp radio initialized");
...
static void nrf91x1_net_if_init(struct net_if *iface)
{
	const struct device *dev = net_if_get_device(iface);
	struct nrf91x1_context *ctx = dev->data;
	LOG_DBG("nrf91x1_net_if_init");

	if (ctx->iface != NULL) {
		/* This driver is only a singleton initialized once for now. */
		LOG_ERR("nrf91x1_net_if_init(%s) failed, init twice not supported for now",
			dev->name);
	} else {
		ctx->iface = iface;
...
static struct nrf91x1_context context = {0};

static const struct dectnrp_driver_api nrf91x1_api = {
	.iface_api = {.init = nrf91x1_net_if_init},
	.start = nrf91x1_start,
	.stop = nrf91x1_stop,
	.get_capabilities = nrf91x1_get_capabilities,
	.configure = nrf91x1_configure,
	.get_time = nrf91x1_get_time,
#if CONFIG_DECTNRP_DRIVER_SCHEDULED_API
	.schedule = nrf91x1_schedule,
#endif
#if CONFIG_DECTNRP_DRIVER_BLOCKING_API
	.set_channel = nrf91x1_set_channel,
	.rx = nrf91x1_rx,
	.tx = nrf91x1_tx,
	.rssi1 = nrf91x1_rssi1,
#endif
...
NET_DEVICE_INIT(dectnrp_nrf91x1, "dectnrp_nrf91x1", nrf91x1_init, NULL, &context, NULL,
		CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &nrf91x1_api, DECTNRP_L2,
